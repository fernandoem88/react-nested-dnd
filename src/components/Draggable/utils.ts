...
export const handleHover = (
  ...
  if (isSameSource) {
    ...
    if (
      ...
      sourceItem.droppableId === destinationItem.droppableId
    ) {
      return
    }
    ...
    // Get vertical middle
    const hoverMiddleY =
      (hoverBoundingRect[marginKeys.last] -
        hoverBoundingRect[marginKeys.first]) /
      2
    ...
    const hoverClientY =
      (clientOffset as XYCoord)[xyCoordinate] -
      hoverBoundingRect[marginKeys.first]
...
export const getTranslationStyle = ({
  ...
  const delta = Math.max(
    margin?.[marginKeys.first] || 0,
    margin?.[marginKeys.last] || 0
  )
  const xyCoordinate = horizontal ? 'x' : 'y'
  ...
  const transform =
    translateSign && domRect
      ? `translate${xyCoordinate.toUpperCase()}(${
          translateSign * (rectSize + delta)
        }px)`
      : undefined

  const pointerEventsProps = isDragging ? { pointerEvents: 'none' } : {}
  const transformProps = transform ? { transform } : {}
  const transitionProps = hasDraggedItem
    ? { transition: '0.3s' }
    : { transition: '0s' }
